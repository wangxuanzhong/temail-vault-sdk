...
void FakeVaultCipher::generateKeyPair(std::string &publicKey, std::string &privateKey, ErrorHandler handler) {
  publicKey = "hello";
  privateKey = "world";
}

void FakeVaultCipher::sign(const char * privateKey, const ByteBuffer &plaintext, ByteBuffer &signature, ErrorHandler handler) {
  char const *token = " of ";
...
bool FakeVaultCipher::verify(const char * publicKey, const ByteBuffer &plaintext, const ByteBuffer &signature, ErrorHandler handler) {
...
void FakeVaultCipher::encrypt(const char * publicKey, const ByteBuffer &plaintext, ByteBuffer &encrypted, ErrorHandler handler) {
  if (strcmp(publicKey, "hello") != 0)
    handler("Failed to encrypt text");
...
void FakeVaultCipher::decrypt(const char * privateKey, const ByteBuffer &encrypted, ByteBuffer &plaintext, ErrorHandler handler) {
  char const *token = "'s ";
